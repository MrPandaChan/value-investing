<template>
  <div class="value-evaluation">
    ...
    <table class="evaluation-table">
      ...
      <tbody>
        <template v-for="(category, categoryIndex) in evaluationData" :key="category.id">
          <!-- 类别标题行 -->
          <tr class="category-separator">
            ...
            <td class="input-col">{{ getCategoryScore(category) }}/{{ getCategoryMaxScore(category) }}</td>
          </tr>
    ...
    if (category) {
      ...
      externalItemKeys.forEach(itemKey => {
        if (!defaultItemKeys.includes(itemKey)) {
          warnings.push(`Extra evaluation item in category ${category.name}: ${itemKey}`)
        }
      })
    }
  })
...
const initializeData = () => {
  ...
  if (props.scoreData) {
    ...
    Object.keys(props.scoreData).forEach(categoryId => {
      const category = data.find((c: EvaluationCategory) => c.id === categoryId)
      if (category) {
        const categoryScores = props.scoreData![categoryId]
        Object.keys(categoryScores).forEach(itemKey => {
          const item = category.items.find((i: EvaluationItem) => i.key === itemKey)
          if (item) {
...
const totalScore = computed(() => {
  return evaluationData.value.reduce((sum, category) => {
    return sum + category.items.reduce((itemSum, item) => itemSum + item.userScore, 0)
  }, 0)
})
...
const totalMaxScore = computed(() => {
  return evaluationData.value.reduce((sum, category) => {
    return sum + category.items.reduce((itemSum, item) => itemSum + item.maxScore, 0)
  }, 0)
})
  ...
  const option = {
    ...
    radar: {
      ...
      splitArea: {
        show: true,
        areaStyle: {
          color: ['rgba(66, 165, 245, 0.05)', 'rgba(66, 165, 245, 0.1)',
                 'rgba(66, 165, 245, 0.15)', 'rgba(66, 165, 245, 0.2)',
